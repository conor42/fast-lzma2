...
BYTE* RMF_structuredAsOutputBuffer(FL2_matchTable* tbl, size_t index)
{
...
void RMF_structuredLimitLengths(FL2_matchTable* tbl, size_t index)
{
    ...
    for (size_t length = 2; length < RADIX_MAX_LENGTH && length <= index; ++length) {
        size_t i = (index - length) >> UNIT_BITS;
        size_t u = (index - length) & UNIT_MASK;
        U32 link = ((RMF_unit*)tbl->table)[i].links[u];
