...
static const FL2_compressionParameters FL2_defaultCParameters[FL2_MAX_CLEVEL + 1] = {
    ...
    { 21, 2, 7, 0, 16, 40, 1, 8, FL2_opt }, /* 5 */
    { 22, 2, 7, 0, 28, 40, 1, 8, FL2_opt }, /* 6 */
    { 23, 2, 8, 0, 40, 48, 1, 8, FL2_opt }, /* 7 */
    { 24, 2, 9, 0, 40, 48, 1, 8, FL2_ultra }, /* 8 */
    { 25, 2, 10, 0, 48, 64, 1, 8, FL2_ultra }, /* 9 */
    { 26, 2, 11, 1, 60, 64, 1, 9, FL2_ultra }, /* 10 */
    { 27, 2, 12, 2, 128, 96, 1, 10, FL2_ultra }, /* 11 */
    { 28, 2, 14, 3, 252, 160, 1, 10, FL2_ultra } /* 12 */
};

static const FL2_compressionParameters FL2_7zCParameters[FL2_MAX_CLEVEL + 1] = {
    { 0,0,0,0,0,0,0 },
    { 20, 1, 7, 0, 6, 32, 1, 8, FL2_fast }, /* 1 */
    { 20, 2, 7, 0, 12, 32, 1, 8, FL2_fast }, /* 2 */
    { 21, 2, 7, 0, 16, 32, 1, 8, FL2_fast }, /* 3 */
    { 20, 2, 7, 0, 16, 32, 1, 8, FL2_opt }, /* 4 */
    { 25, 2, 9, 0, 40, 64, 1, 8, FL2_ultra }, /* 5 */
    { 26, 2, 10, 1, 60, 64, 1, 9, FL2_ultra }, /* 6 */
    { 27, 3, 12, 1, 128, 128, 1, 10, FL2_ultra }, /* 7 */
    { 28, 3, 13, 2, 128, 160, 1, 10, FL2_ultra }, /* 8 */
    { 28, 3, 14, 3, 252, 160, 1, 10, FL2_ultra }, /* 9 */
};
...
{
    FL2_lzma2Parameters* cParams = &cctx->params.cParams;
    RMF_parameters* rParams = &cctx->params.rParams;
    cParams->lc = 3;
    cParams->lp = 0;
    cParams->pb = 2;
    ...
    rParams->dictionary_log = params->windowLog;
...
static void FL2_buildRadixTable(void* jobDescription)
{
    const FL2_job* const job = (FL2_job*)jobDescription;
    FL2_CCtx* cctx = job->cctx;

    /* build */
    RMF_buildTable(cctx->matchTable, job->num, 1, cctx->curBlock.data, cctx->curBlock.start, cctx->curBlock.end);
}

/* FL2_compressRadixChunk() : POOL_function type */
static void FL2_compressRadixChunk(void* jobDescription)
{
    const FL2_job* const job = (FL2_job*)jobDescription;
    FL2_CCtx* cctx = job->cctx;

    unsigned num = job->num;

    cctx->jobs[num].cSize = FL2_lzma2Encode(cctx->jobs[num].enc, cctx->matchTable, job->block, &cctx->params.cParams, 0);
}

static void FL2_initEncoders(FL2_CCtx* cctx)
{
    for(unsigned u = 0; u < cctx->jobCount; ++u) {
        FL2_lzma2HashAlloc(cctx->jobs[u].enc, &cctx->params.cParams);
    }
}

static size_t FL2_compressCurBlock(FL2_CCtx* cctx)
{
    size_t encodeSize = (cctx->curBlock.end - cctx->curBlock.start);

#ifdef FL2_MULTITHREAD
    size_t mfThreads = encodeSize / RMF_MIN_BYTES_PER_THREAD;
    ...
    FL2_initEncoders(cctx); /* Create hash objects together, leaving the (large) match table last */
    ...
    {   size_t sliceStart = cctx->curBlock.start;
        size_t sliceSize = (encodeSize + nbThreads - 1) / nbThreads;
        size_t lastSliceSize = encodeSize - sliceSize;
    ...
    for (size_t u = 1; u < mfThreads; ++u) {
            POOL_add(cctx->factory, FL2_buildRadixTable, &cctx->jobs[u]);   /* this call is blocking when thread worker pool is exhausted */
        }
#endif

    RMF_buildTable(cctx->matchTable, 0, mfThreads > 1, cctx->curBlock.data, cctx->curBlock.start, cctx->curBlock.end);
    ...
    {   size_t err = RMF_integrityCheck(cctx->matchTable, cctx->curBlock.data, cctx->curBlock.start, cctx->curBlock.end, cctx->params.rParams.depth);
        if (err)
...
static void FL2_beginFrame(FL2_CCtx* ctx)
{
    ctx->dictMax = 0;
}

static size_t FL2_compressBlock(FL2_CCtx* cctx,
    ...
    FL2_writerFn writeFn, void* opaque)
{
    BYTE* dstBuf = dst;
    size_t outSize = 0;
    size_t dictionary_size = (size_t)1 << cctx->params.rParams.dictionary_log;
    ...
    while (srcSize > 0) {
        ...
        nbThreads = FL2_compressCurBlock(cctx);
...
{
    BYTE* dstBuf = dst;
    BYTE* end = dstBuf + dstCapacity;
    size_t cSize = 0;

    if (compressionLevel > FL2_MAX_CLEVEL) compressionLevel = FL2_MAX_CLEVEL;
    if (compressionLevel > 0) {
        FL2_fillParameters(cctx, &FL2_defaultCParameters[compressionLevel]);
    }

    DEBUGLOG(4, "FL2_compressCCtx : level %u, %u src => %u avail", cctx->params.compressionLevel, (U32)srcSize, (U32)dstCapacity);

    if (dstCapacity < 2 - cctx->params.omitProp) /* empty LZMA2 stream is byte sequence {0, 0} */
        return FL2_ERROR(dstSize_tooSmall);
...
FL2LIB_API size_t FL2_compressCCtxBlock(FL2_CCtx* cctx,
    ...
    const void* src, size_t srcStart, size_t srcSize)
{
    return FL2_compressBlock(cctx, src, srcStart, srcSize, dst, dstCapacity, NULL, NULL);
}
...
FL2LIB_API size_t FL2_compressCCtxBlock_toFn(FL2_CCtx* cctx,
    ...
    const void* src, size_t srcStart, size_t srcSize)
{
    return FL2_compressBlock(cctx, src, srcStart, srcSize, NULL, 0, writeFn, opaque);
}
    ...
    unsigned nbThreads)
{
    ...
    cctx = FL2_createCCtxMt(nbThreads);
    if (cctx == NULL)
        return FL2_ERROR(memory_allocation);
...
FL2LIB_API size_t FL2_CCtx_setParameter(FL2_CCtx* cctx, FL2_cParameter param, unsigned value)
{
    ...
    case FL2_p_dictionaryLog:
        if (value) {  /* 0 : does not change current dictionaryLog */
            CLAMPCHECK(value, FL2_WINDOWLOG_MIN, FL2_WINDOWLOG_MAX);
...
FL2LIB_API FL2_CStream* FL2_createCStream(void)
{
    ...
    FL2_CStream* fcs = malloc(sizeof(FL2_CStream));
...
static size_t FL2_compressStream_internal(FL2_CStream* fcs,
    FL2_outBuffer* output, int ending)
{
    FL2_CCtx* cctx = fcs->cctx;
    size_t out_start = output->pos;
    if (output->pos >= output->size)
        return 0;
    if (fcs->out_thread == fcs->thread_count) {
        if (fcs->inBuff.start < fcs->inBuff.end) {
            if (fcs->cctx->params.doXXH && !cctx->params.omitProp) {
            ...
            fcs->out_thread = 0;
            fcs->thread_count = FL2_compressCurBlock(cctx);
    ...
    for (; fcs->out_thread < fcs->thread_count; ++fcs->out_thread) {
        ...
        BYTE* dstBuf = (BYTE*)output->dst + output->pos;
        size_t dstCapacity = output->size - output->pos;
...
static size_t FL2_remainingOutputSize(FL2_CStream* fcs)
{
    FL2_CCtx* cctx = fcs->cctx;
...
{
    FL2_blockBuffer* inBuff = &fcs->inBuff;
    FL2_CCtx* cctx = fcs->cctx;
    size_t block_overlap = OVERLAP_FROM_DICT_LOG(cctx->params.rParams.dictionary_log, cctx->params.rParams.overlap_fraction);
    if (FL2_isError(fcs->thread_count))
        return fcs->thread_count;
    if (output->pos < output->size) while (input->pos < input->size) {
        ...
        if (inBuff->start > block_overlap && input->pos < input->size) {
            size_t from = (inBuff->end - block_overlap) & ~(sizeof(size_t) - 1);
            size_t overlap = inBuff->end - from;
            if (overlap <= from) {
        ...
        if (fcs->out_thread == fcs->thread_count) {
            size_t toRead = MIN(input->size - input->pos, inBuff->bufSize - inBuff->end);
        ...
        if (inBuff->end == inBuff->bufSize || fcs->out_thread < fcs->thread_count) {
            CHECK_F(FL2_compressStream_internal(fcs,
                output, 0));
        }
        if (fcs->out_thread < fcs->thread_count)
            break;
...
FL2LIB_API size_t FL2_endStream(FL2_CStream* fcs, FL2_outBuffer* output)
{
    ...
    if (fcs->cctx->params.doXXH && !fcs->cctx->params.omitProp && fcs->hash_pos < XXHASH_SIZEOF) {
        size_t to_write = MIN(output->size - output->pos, XXHASH_SIZEOF - fcs->hash_pos);
        XXH32_canonical_t canonical;
