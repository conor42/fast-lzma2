...
typedef struct {
    unsigned windowLog;        /* largest match distance : larger == more compression, more memory needed during decompression; >= 27 == more memory, slower */
    ...
    unsigned bufferLog;        /* buffer size for processing match chains is (windowLog - bufferLog) : when divideAndConquer enabled, affects compression; */
